...
SELECT a.name account_name, w.occurred_at dates, w.channel channel
FROM accounts a
JOIN web_events w
...
SELECT a.name primaryContact, w.occurred_at TimeHappened
FROM web_events w
...
SELECT a.name, MIN(total_amt_usd) totalUSD
FROM orders o
...
across their orders. Your result should have four columns 
- one for the account name and one for the average quantity purchased for 
each of the paper types for each account.

SELECT a.name , AVG(o.standard_qty) standard_qty, AVG(o.poster_qty) poster_qty, AVG(o.gloss_qty) gloss_amt_usd
FROM accounts a
JOIN orders o ON a.id = o.account_id
GROUP BY a.name
...
SELECT a.name, AVG(o.standard_amt_usd) standard_amt_usd, AVG(o.poster_amt_usd) poster_amt_usd, AVG(o.gloss_amt_usd) gloss_amt_usd
FROM accounts a
JOIN orders o ON a.id = o.account_id
GROUP BY a.name
...
SELECT s.name salesREP, w.channel, COUNT(channel) number_of_occurrences
FROM sales_reps s
JOIN accounts a ON s.id = a.sales_rep_id
JOIN web_events w ON w.account_id = a.id
GROUP BY salesREP, w.channel
...
SELECT r.name Region, channel, COUNT(channel) number_of_occurrences
FROM sales_reps s
JOIN accounts a ON s.id = a.sales_rep_id
JOIN web_events w ON w.account_id = a.id
JOIN region r ON s.region_id = r.id
...
--QUIZ: DISTINCT (IT DOES SLOW DOWN QUERIES)
...
SELECT o.id, o.total_amt_usd,
CASE 
      WHEN total_amt_usd >= 3000 THEN 'Large'
      ELSE 'Small' END AS LevelOfOrder
FROM orders o;
...
SELECT a.name AccountName, SUM(total_amt_usd) TotalSalesOfAllOrders, 
CASE WHEN SUM(total_amt_usd) < 100000 THEN 'Lower Level'
      WHEN SUM(total_amt_usd) < 200000 AND SUM(total_amt_usd) >=100000 THEN 'Middle Level'
      WHEN SUM(total_amt_usd) >= 200000 THEN 'Upper Level' END AS TopLevel
FROM orders o
JOIN accounts a ON a.id = o.account_id
GROUP BY a.name
...
by customers only in 2016 and 2017. Keep the same levels as in the previous question. 
Order with the top spending customers listed first.
